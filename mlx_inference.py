"""MLX-based deepfake detection inference script.

Port of perf_final.py to use MLX framework for improved performance on Apple Silicon.
"""
...
from effort_detector_mlx import EffortDetector
...
class DeviceManager:
    """Manages MLX device selection."""

    @staticmethod
    def get_device() -> str:
        """Get MLX device (unified memory model)."""
...
class ModelLoader:
    """Handles MLX detector model loading."""

    @staticmethod
    def load_detector(weights: str) -> EffortDetector:
        """Load and configure MLX detector model."""
...
class ImagePreprocessor:
    ...
    @staticmethod
    def preprocess_face(img_bgr: np.ndarray) -> mx.array:
...
class InferenceEngine:
    """Handles model inference with timing."""
    ...
    def infer_single_image(
        ...
        face_tensor = ImagePreprocessor.preprocess_face(face_aligned)
        data = {"image": face_tensor}

        # Run inference
        predictions = model(data, inference=True)

        cls_out = predictions["cls"]
        prob = predictions["prob"]

        # Convert to numpy for compatibility
        cls_out = np.argmax(cls_out[0].tolist())
        prob = float(prob[0])
...
class ResultWriter:
    ...
    def __init__(self, filename: str = "inference_results_mlx.txt") -> None:
    ...
    def _write_header(self, f: IO) -> None:
        """Write file header."""
        f.write("MLX Deepfake Detection Results\n")
        f.write("=" * 80 + "\n\n")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="MLX deepfake image inference",
    )
    parser.add_argument("--weights", required=True, help="MLX model weights (.npz)")
...
def main() -> None:
    ...
    # Load model
    model = ModelLoader.load_detector(args.weights)
    ...
    if inference_times:
        ...
        logger.info("Average inference time per image: %.4f seconds", avg_time)
