    ...
    def train_epoch(
        ...
        for iteration, data_dict in tqdm(
            ...
            if (step_cnt + 1) % test_step == 0:
                if (test_data_loaders is not None and (not self.config["ddp"])) or (
                    test_data_loaders is not None
                    and (self.config["ddp"] and dist.get_rank() == 0)
                ):
    ...
    def save_best(
        ...
        if "pred" in metric_one_dataset:
            acc_real, acc_fake = self.get_respect_acc(
                metric_one_dataset["pred"],
                metric_one_dataset["label"],
            )
