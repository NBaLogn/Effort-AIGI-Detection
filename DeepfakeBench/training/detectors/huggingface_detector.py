...
class HuggingFaceCLIPDetector(nn.Module):
    ...
    def features(self, data_dict: dict) -> torch.tensor:
        ...
        # Get image input - could be PIL images or tensors
        images = data_dict['image']

        # If images are tensors, convert to PIL for CLIP processor
        if isinstance(images, torch.Tensor):
            # Convert tensor back to PIL images for CLIP processing
            ...
            # Assume PIL images or already processed inputs
            if self.processor is None:
                from transformers import AutoProcessor
                self.processor = AutoProcessor.from_pretrained(
                    getattr(self.config, 'huggingface_model_name', 'openai/clip-vit-large-patch14')
