...
def generate_dataset_file(
    ...
    if (
        ...
        or dataset_name == "DeepFakeDetection"
        or dataset_name == "FaceShifter"
    ):
        ff_dict = {
            "Deepfakes": "FF-DF",
            "Face2Face": "FF-F2F",
        ...
        with open(
            file=os.path.join(
                os.path.join(dataset_root_path, "FaceForensics++", "train.json"),
            ),
        ) as f:
            train_json = json.load(f)
        with open(
            file=os.path.join(
                os.path.join(dataset_root_path, "FaceForensics++", "val.json"),
            ),
        ) as f:
            val_json = json.load(f)
        with open(
            file=os.path.join(
                os.path.join(dataset_root_path, "FaceForensics++", "test.json"),
            ),
        ) as f:
        ...
        if os.path.isdir(dataset_path) and os.path.isdir(
            os.path.join(dataset_path, "original_sequences"),
        ):
            label = "Real"
            ...
            for compression_level in os.scandir(
                os.path.join(dataset_path, "original_sequences", "youtube"),
            ):
                if compression_level.is_dir():
                    ...
                    for video_path in os.scandir(
                        ...
                        if video_path.is_dir():
                            ...
                            dataset_dict["FaceForensics++"]["FF-real"][mode][
                                compression_level
                            ][video_name] = {
                                "label": ff_dict[label],
                                "frames": frame_paths,
            ...
            for compression_level in os.scandir(
                os.path.join(dataset_path, "original_sequences", "actors"),
            ):
                if compression_level.is_dir() and compression_level.name in [
                    "c23",
                    "c40",
                    "raw",
                ]:
        ...
        if os.path.isdir(os.path.join(dataset_path, "manipulated_sequences")):
            for label_dir in os.scandir(
                os.path.join(dataset_path, "manipulated_sequences"),
            ):
                if label_dir.is_dir():
                    ...
                    # Iterate over all compression levels: c23, c40, raw
                    for compression_level in os.scandir(
                        os.path.join(dataset_path, "manipulated_sequences", label),
                    ):
                        if compression_level.is_dir() and compression_level.name in [
                            "c23",
                            "c40",
                                ...
                                if video_path.is_dir():
                                    ...
                                    if label != "FaceShifter":
                                        ...
                                        if os.path.exists(mask_paths):
                                            mask_frames_paths = [
                                                os.path.join(mask_paths, frame.name)
                                                for frame in os.scandir(mask_paths)
                                            ]
                                        else:
                                            mask_frames_paths = []
                                        try:
                                            ...
                                            dataset_dict["FaceForensics++"][
                                                ff_dict[label]
                                            ][mode][compression_level][video_name] = {
                                                "label": ff_dict[label],
                                                "frames": frame_paths,
                                        ...
                                        # DeepfakeDetection dataset
                                            ...
                                            dataset_dict["FaceForensics++"][
                                                ff_dict[label]
                                            ]["val"][compression_level][video_name] = {
                                                "label": ff_dict[label],
                                                "frames": frame_paths,
                                                "masks": mask_frames_paths,
                                            }
                                            dataset_dict["FaceForensics++"][
                                                ff_dict[label]
                                            ]["test"][compression_level][video_name] = {
                                                "label": ff_dict[label],
                                                "frames": frame_paths,
                                                "masks": mask_frames_paths,
                                            }
        ...
        if dataset_name == "FaceForensics++":
            ...
            if (
                "DFD_fake" in dataset_dict["FaceForensics++"]
                and "DFD_real" in dataset_dict["FaceForensics++"]
            ):
            ...
            if (
                "FF-FH" in dataset_dict["FaceForensics++"]
                and "FF-real" in dataset_dict["FaceForensics++"]
            ):
                # Add the DeepfakeDetection dataset to the dataset_dict
                ...
                # TODO
                raise ValueError(
                    "DeepfakeDetection dataset not found in FaceForensics++ dataset.",
                )
        else:
        ...
        if dataset_name == "FaceForensics++":
            for label, value in dataset_dict["FaceForensics++"].items():
                if label != "FF-real":
                    with open(
                        os.path.join(output_file_path, f"{label}.json"),
                        "w",
                    ) as f:
                        data = {
                            label: {
    ...
    ## Note: videos in Celeb-DF-v1/2 are not in the same format as in FaceForensics++ dataset
        ...
        for folder in os.scandir(dataset_path):
            ...
            assert label in ["CelebDFv1_real", "CelebDFv1_fake"], (
            ...
            for video_path in os.scandir(
                os.path.join(dataset_path, folder.name, "frames"),
            ):
                if video_path.is_dir():
                    video_name = video_path.name
                    frame_paths = [
                    ...
                    dataset_dict[dataset_name][label]["train"][video_name] = {
                        "label": label,
                        "frames": frame_paths,
                    }
        ...
        with open(
            os.path.join(dataset_root_path, dataset_name, "List_of_testing_videos.txt"),
        ) as f:
            lines = f.readlines()
        for line in lines:
            if "real" in line:
            ...
            dataset_dict[dataset_name][label]["val"][vidname] = {
                "label": label,
                "frames": frame_paths,
            }
    ...
    ## Note: videos in Celeb-DF-v1/2 are not in the same format as in FaceForensics++ dataset
        ...
        for folder in os.scandir(dataset_path):
            ...
            assert label in ["CelebDFv2_real", "CelebDFv2_fake"], (
                f"Invalid label: {label}"
            )
            dataset_dict[dataset_name][label] = {}
            dataset_dict[dataset_name][label]["train"] = {}
    ...
    ## DFDCP dataset
        ...
        for dataset in dataset_info.keys():
            ...
            if Path(os.path.join(dataset_path, index, "frames", vidname)).exists():
                frame_paths = glob.glob(
                    os.path.join(dataset_path, index, "frames", vidname, "*png"),
                )
                ...
                if label == "real":
                    ...
                    raise ValueError(
                        f"wrong in processing vidname {dataset_name}: {line}",
                    )
                set_attr = dataset_info[dataset]["set"]  # train, test, val
                dataset_dict[dataset_name][label][set_attr][vidname] = {
                    "label": label,
                    "frames": frame_paths,
                }
        # Special case for val data of DFDCP
        for label in ["DFDCP_Real", "DFDCP_FakeA", "DFDCP_FakeB"]:
            dataset_dict[dataset_name][label]["val"] = dataset_dict[dataset_name][
                label
            ]["test"]
        ...
        for folder in os.scandir(dataset_path):
            ...
            if folder.name in ["test"]:
                ...
                for index, row in df.iterrows():
                    ...
                    assert label in ["DFDC_Real", "DFDC_Fake"], (
                        f"Invalid label: {label}"
                    )
                    frame_paths = glob.glob(
                        os.path.join(
                ...
                num_file = 0
                for dfdc_train_part in os.scandir(
                    os.path.join(dataset_path, folder.name),
                ):
                    if not os.path.isdir(dfdc_train_part):
                        continue
                    num_file += 1
                    print(f"processing {num_file}th file in 50 files.")
                    with open(os.path.join(dfdc_train_part, "metadata.json")) as f:
                        metadata = json.load(f)
                    for video_path in os.scandir(
                        os.path.join(dfdc_train_part, "frames"),
                    ):
                        if video_path.is_dir():
                            video_name = video_path.name
                            ...
                            dataset_dict[dataset_name][label]["val"][video_name] = {
                                "label": label,
                                "frames": frame_paths,
                            }
    ...
    ## DeeperForensics-1.0 dataset
        ...
        with open(
            os.path.join(dataset_root_path, dataset_name, "lists/splits/train.txt"),
        ) as f:
            train_txt = f.readlines()
            train_txt = [line.strip().split(".")[0] for line in train_txt]
        with open(
            os.path.join(dataset_root_path, dataset_name, "lists/splits/test.txt"),
        ) as f:
            test_txt = f.readlines()
            test_txt = [line.strip().split(".")[0] for line in test_txt]
        with open(
            os.path.join(dataset_root_path, dataset_name, "lists/splits/val.txt"),
        ) as f:
            val_txt = f.readlines()
        ...
        if not Path(
            os.path.join(dataset_path, "manipulated_videos", perturbation),
        ).exists():
            raise ValueError(
                f"wrong in processing perturbation {perturbation} in manipulated_videos",
            )
        print(f"processing perturbation {perturbation} in manipulated_videos")
        for video_path in os.scandir(
            os.path.join(dataset_path, "manipulated_videos", perturbation, "frames"),
        ):
            if video_path.is_dir():
                video_name = video_path.name
                ...
                dataset_dict[dataset_name][label][set_attr][video_name] = {
                    "label": label,
                    "frames": frame_paths,
                }
        for actor_path in os.scandir(os.path.join(dataset_path, "source_videos")):
            ...
            label = "DF_real"
            video_paths = [
                os.path.join(actor_path, "frames", video.name)
                for video in os.scandir(os.path.join(actor_path, "frames"))
            ]
            for video_path in video_paths:
                ...
                dataset_dict[dataset_name][label]["test"][video_name] = {
                    "label": label,
                    "frames": frame_paths,
                }
                dataset_dict[dataset_name][label]["val"][video_name] = {
